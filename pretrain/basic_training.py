...
class TrainingConfig:
    ...
    def __init__(self):
        ...
        self.add_math_tokens = get_env_variable("ADD_MATH_TOKENS", "true", bool)
    ...
    def log_config(self):
        ...
        logger.info(f"Add Math Tokens: {self.add_math_tokens}")
...
def get_turkish_math_tokens():
    """
    Returns essential tokens to add to tokenizer.
    Focuses on: multi-word phrases, symbols, subscripts/superscripts, Greek letters, and key LaTeX commands.
    Avoids adding every inflected form of Turkish words.
    """
    
    # CRITICAL: Multi-word phrases that should be single tokens
    # These have the biggest impact on tokenization quality
    multi_word_phrases = [
        # Very high frequency phrases
        'birim vektör', 'normal vektör', 'teğet doğrusu',
        'çapraz çarpım', 'iç çarpım', 'dış çarpım', 'vektörel çarpım', 'skaler çarpım',
        'yönlü türev', 'kısmi türev', 'toplam türev',
        'değişim oranı', 'anlık değişim', 'ortalama değişim',
        'tanım kümesi', 'görüntü kümesi', 'çözüm kümesi',
        'üç boyutlu', 'iki boyutlu', 'n boyutlu',
        'teğet doğru', 'normal doğru',
        
        # Common connectives
        'sonuç olarak', 'dolayısıyla', 'örneğin',
        'bulmak için', 'hesaplamak için', 'elde etmek', 'elde ederiz', 'elde edilir',
        'her bir', 'her iki', 'şu şekilde', 'bu şekilde',
        'adım adım', 'aşağıdaki gibi', 'yukarıdaki gibi',
    ]
    
    # Mathematical symbols (only those in your corpus)
    symbols = [
        # Basic operations
        '√', '±', '×', '÷',
    ...
    # Subscripts and superscripts (very important!)
    ...
    # Greek letters (only frequently used ones from your corpus)
    greek = [
        # Lowercase (high frequency in your corpus)
        'α', 'β', 'γ', 'δ', 'ε', 'ζ', 'η', 'θ', 'ι', 'κ',
        'λ', 'μ', 'ν', 'ξ', 'ο', 'π', 'ρ', 'σ', 'τ', 'υ',
        'φ', 'χ', 'ψ', 'ω',
        
        # Uppercase (only commonly used ones)
        'Α', 'Β', 'Γ', 'Δ', 'Ε', 'Ζ', 'Η', 'Θ', 'Ι', 'Κ',
        'Λ', 'Μ', 'Ν', 'Ξ', 'Ο', 'Π', 'Ρ', 'Σ', 'Τ', 'Υ',
        'Φ', 'Χ', 'Ψ', 'Ω',
    ]
    
    # LaTeX commands (only high-frequency ones from your corpus)
    latex = [
        # Basic formatting
        '\\frac', '\\dfrac', '\\sqrt',
        '\\left', '\\right',
        '\\text', '\\mathrm', '\\mathbf', '\\mathbb', '\\mathcal', '\\mathfrak',
        ...
        # Accents
        '\\hat', '\\bar', '\\tilde', '\\vec', '\\dot', '\\ddot', '\\overline',
        ...
        # Environments (only most common)
        '\\begin', '\\end',
    ]
    
    # Number sets (very common in math)
    number_sets = [
        'ℕ', 'ℤ', 'ℚ', 'ℝ', 'ℂ',
    ]
...
def apply_single_digit_splitting(tokenizer):
    """Forces the tokenizer to treat ASCII digits (0-9) as isolated tokens."""
    logger.info("Applying single-digit splitting rule...")
    
    # Define the rule: Split on any digit, mark it as isolated
    digit_splitter = Split(
        pattern=Regex(r"[0-9]"), 
        behavior="isolated", 
        invert=False
    )

    # Chain with existing rules
    current_pre_tokenizer = tokenizer._tokenizer.pre_tokenizer
    if isinstance(current_pre_tokenizer, pre_tokenizers.Sequence):
        new_pre_tokenizer = Sequence(current_pre_tokenizer.pre_tokenizers + [digit_splitter])
    else:
        new_pre_tokenizer = Sequence([current_pre_tokenizer, digit_splitter])
        
    tokenizer._tokenizer.pre_tokenizer = new_pre_tokenizer
    return tokenizer

def extend_tokenizer_with_math(tokenizer, model):
    tokenizer = apply_single_digit_splitting(tokenizer)
    # Verify it worked
    test = tokenizer.tokenize("123")
    logger.info(f"Digit splitting verification: '123' → {test}")
    
    math_tokens = get_turkish_math_tokens()
    initial_vocab_size = len(tokenizer)
    
    num_added = tokenizer.add_tokens(math_tokens)
    new_vocab_size = len(tokenizer)
    
    logger.info(f"  Initial vocab: {initial_vocab_size:,}")
    logger.info(f"  Added tokens: {num_added}")
    logger.info(f"  New vocab: {new_vocab_size:,}")
    
    # Calculate sizes
    current_model_vocab_size = model.get_input_embeddings().weight.shape[0]
    logger.info(f"Tokenizer size: {new_vocab_size} | Model embedding size: {current_model_vocab_size}")

    # Resize if ANY mismatch exists (not just if we added new tokens)
    if new_vocab_size != current_model_vocab_size:
        logger.info(f"Resizing model embeddings from {current_model_vocab_size} to {new_vocab_size}")
        
        # Store old embeddings for smart initialization
        ...
        # Smart Initialization (Only if we grew the model)
        if new_vocab_size > current_model_vocab_size:
            new_input_embeddings = model.get_input_embeddings().weight.data
            new_output_embeddings = model.get_output_embeddings().weight.data
            
            mean_input_emb = old_input_embeddings.mean(dim=0)
            mean_output_emb = old_output_embeddings.mean(dim=0)
            
            # Initialize all new slots (from the old size up to the new size)
            for i in range(current_model_vocab_size, new_vocab_size):
                new_input_embeddings[i] = mean_input_emb + torch.randn_like(mean_input_emb) * 0.02
                new_output_embeddings[i] = mean_output_emb + torch.randn_like(mean_output_emb) * 0.02
            
            logger.info("✓ Model embeddings resized and initialized smartly")
        
        # Verify initialization
        logger.info(f"  Old embedding norm: {old_input_embeddings.norm():.4f}")
        logger.info(f"  New tokens embedding norm: {new_input_embeddings[initial_vocab_size:].norm():.4f}")
    else:
        logger.info("Model embeddings are already the correct size.")
...
def main():
    ...
    if config.add_math_tokens:
        logger.info("=" * 80)
        logger.info("EXTENDING TOKENIZER WITH MATH TOKENS")
        logger.info("=" * 80)
        
        tokenizer, model, num_added_tokens = extend_tokenizer_with_math(tokenizer, model)
        
        if num_added_tokens > 0:
            logger.info(f"✓ Successfully added {num_added_tokens} new math tokens")
            logger.info(f"✓ Model now has {model.num_parameters():,} parameters")
            
            # Verify math tokens
            test_expr = "π ≈ 3.14159"
            tokens = tokenizer.tokenize(test_expr)
            logger.info(f"  Test: '{test_expr}' → {tokens}")
            
            extended_tokenizer_path = f"{config.output_dir}/extended_tokenizer"
            os.makedirs(extended_tokenizer_path, exist_ok=True)
            tokenizer.save_pretrained(extended_tokenizer_path)
            logger.info(f"✓ Extended tokenizer saved to: {extended_tokenizer_path}")
        
        logger.info("=" * 80)
    else:
        logger.info("Math token extension disabled (ADD_MATH_TOKENS=false)")
